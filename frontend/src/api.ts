...
export const api = {
  ...
  addTask: (data: { title: string; topic_uid: string; deadline: string | null; priority: string }) => request<Task>("/tasks", { method: "POST", body: JSON.stringify(data) }),
  updateTask: (id: string, data: Partial<Task>) => request<Task>(`/tasks/${id}`, { method: "PATCH", body: JSON.stringify(data) }),
  deleteTask: (id: string) => request<void>(`/tasks/${id}`, { method: "DELETE" }),
  addSession: (data: { subject_uid: string; duration_minutes: number; notes: string | null }) => request<Session>("/study-sessions", { method: "POST", body: JSON.stringify(data) }),
};
